function [masterraster,MDLdata,slowangles,deltaangles,FzdeltaZval,FzslowZval]=RONSanalysisBraincomm(RONSchan,hfo_table)
...
%Algo
%iterate i through each unit in RONSchan i=unit
...
for i=1:numel(RONSchan(:,1)) 
    unit_t=i
    collection = "yuvalHFO";
    
    if strcmp('398',RONSchan.A{i});
        patient_t=1;
    end;
    if strcmp('406',RONSchan.A{i});
        patient_t=2;
    end;
    if strcmp('416',RONSchan.A{i});
        patient_t=3;
    end;
    if strcmp('417',RONSchan.A{i});
        patient_t=4;
    end;
    if strcmp('423',RONSchan.A{i});
        patient_t=5;
    end;

    if i>1 
        if ~strcmp(RONSchan.B{i},RONSchan.B{i-1})
            electrode_t=electrode_t+1;
            [idx]=find(strcmp(RONSchan.A{i},hfo_table.patient)==1);
            [idx2]=find(strcmp(RONSchan.B{i},hfo_table.electrode)==1);
            [int]=intersect(idx,idx2);
            HFO_level_t=hfo_table.levels(int);
            HFOstats=1;
            test_query=['{"patient_id":"' RONSchan.A{i} '","type":"' num2str(2) '","electrode":"' RONSchan.B{i} '" }']; 
        else
            HFOstats=0;
        end;
     else
    ...
    if HFOstats==1
            ...
            parfor j=1:numel(HFO(:,1))
                ...
                finish_t(j)=getfield(HFO,{j},'finish_t');
           ...
           % R before spike
           test_query=['{"patient_id":"' RONSchan.A{i} '","type":"' num2str(1) '","electrode":"' RONSchan.B{i} '" }'];
           ...
           % FR before spike
           test_query=['{"patient_id":"' RONSchan.A{i} '","type":"' num2str(4) '","electrode":"' RONSchan.B{i} '" }'];
...
tempstr=num2str(RONSchan.C{i});
test_query=['{"patient_id":"' RONSchan.A{i} '","channame":"' RONSchan.B{i} '","unitnum":' tempstr ' }'];
...
for j=1:numel(HFO(:,1))
       counter=counter+1;
       unit_ts=round(unit_ts,3); %bug fix
       HFO_start=start_t(j);
       ...
       FzdeltaZval{counter}=deltaZ(IA2);
       FzslowZval{counter}=slowZ(IA2);
       raster(IB)=1;
       k = gausswin(101);
       z1 = conv(raster,k);
...
MDLdata=table(bl,hfo,hfodiff,patient,electrode,unit,loc,soz,HFOlevel,power,frequency,preR,preFR,FzslowZ,pcode);
