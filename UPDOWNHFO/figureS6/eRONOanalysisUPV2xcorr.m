function [masterraster,masterraster_pair,MDLdata,MDLdata_pair]=eRONOanalysisUPV2xcorr(RONOchan1,RONOchan2,hfo_table,output_t_maxR,unitparam)
...
for i=1:numel(RONOchan1(:,1)) 
    i
    unit_t=i;
    collection = "yuvalHFOn";
    
    if strcmp('398',RONOchan1.A{i});
        patient_t=1;
    end;
    if strcmp('406',RONOchan1.A{i});
        patient_t=2;
    end;
    if strcmp('416',RONOchan1.A{i});
        patient_t=3;
    end;
    if strcmp('417',RONOchan1.A{i});
        patient_t=4;
    end;
    if strcmp('423',RONOchan1.A{i});
        patient_t=5;
    end;

    if i>1 
        if ~strcmp(RONOchan1.B{i},RONOchan1.B{i-1})
            electrode_t=electrode_t+1;
            [idx]=find(strcmp(RONOchan1.A{i},hfo_table.patient)==1);
            [idx2]=find(strcmp(RONOchan1.B{i},hfo_table.electrode)==1);
    ...
    if HFOstats==1
            ...
            test_query=['{"patient_id":"' RONOchan1.A{i} '","type":"' num2str(1) '","electrode":"' RONOchan1.B{i} '" }']; 
       ...
       [idx_str]=strcmp(RONOchan1.A{i},output_t_maxR.patient);
       [idx2_str]=strcmp(RONOchan1.B{i},output_t_maxR.electrode);
       ...
       slowangle_t=tempangles_sub;      
           ...
           % FR before spike
           collection = "yuvalHFO";
           test_query=['{"patient_id":"' RONOchan1.A{i} '","type":"' num2str(2) '","electrode":"' RONOchan1.B{i} '" }'];
           ...
           test_query=['{"patient_id":"' RONOchan1.A{i} '","type":"' num2str(3) '","electrode":"' RONOchan1.B{i} '" }'];
           ...
           test_query=['{"patient_id":"' RONOchan1.A{i} '","type":"' num2str(5) '","electrode":"' RONOchan1.B{i} '" }'];
...
tempstr=num2str(RONOchan1.C{i});
test_query=['{"patient_id":"' RONOchan1.A{i} '","channame":"' RONOchan1.B{i} '","unitnum":' tempstr ' }'];
...
for i=1:numel(RONOchan2(:,1)) 
    i
    unit_t=i;
    collection = "yuvalHFOn";
    
    if strcmp('398',RONOchan2.A{i});
        patient_t=1;
    end;
    if strcmp('406',RONOchan2.A{i});
        patient_t=2;
    end;
    if strcmp('416',RONOchan2.A{i});
        patient_t=3;
    end;
    if strcmp('417',RONOchan2.A{i});
        patient_t=4;
    end;
    if strcmp('423',RONOchan2.A{i});
        patient_t=5;
    end;
...
collection = "yuvalUNITn";
tempstr=num2str(RONOchan2.C{i});
test_query=['{"patient_id":"' RONOchan2.A{i} '","channame":"' RONOchan2.B{i} '","unitnum":' tempstr ' }'];
        units = find(conn,collection,'query',test_query);
        unit_ts=[];
        ieegslowphase=[];
        slowZ=[];
...
masterraster_pair=vertcat(masterraster_pair, dsraster_t);
collection = "yuvalHFOn";
end;
MDLdata_pair=table(bl,hfo,hfodiff,patient,electrode,unit,loc,soz,power,frequency,slowangle,bltheta_adj,preSPIKE,preSPIKE2);
MDLdata_pair.power=log10(MDLdata_pair.power);