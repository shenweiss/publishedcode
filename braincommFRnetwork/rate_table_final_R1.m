...
RESP={'2061','4122','4145','4163','4166','449','458','463','466','468','473','477','478','481','IO004','IO006','IO013','IO014'}
NONRESP={'4100','4110','456','479','480','IO005','IO010','IO015','IO027'}
E1={'4122', '4124', '4145', '4166', '466', '468', '477', 'IO004'};
...
for i=1:numel(patients)
    ...
    file_block = distinct(conn,collection,"file_block",'Query',test_query);
        for j=1:numel(file_block)
            test_query=['{"patient_id":"' patients{i} '","file_block":"' file_block{j} '","type":"1"}'];
            ...
            rates=[];
               for k=1:numel(electrodes)                  
                 ...
                 chan{k}=electrodes{k};
                  for l=1:3
                    if l==1  
                    test_query=['{"patient_id":"' patients{i} '","electrode":"' electrodes{k} '","file_block":"' file_block{j} '","type":"1"}'];
                    end;
                    if l==2
                    test_query=['{"patient_id":"' patients{i} '","electrode":"' electrodes{k} '","file_block":"' file_block{j} '","type":"4" }'];
                    end;
                    if l==3
                    test_query=['{"patient_id":"' patients{i} '","electrode":"' electrodes{k} '","file_block":"' file_block{j} '","type":"4","freq_pk": {$gt:350} }'];
                    end;
                 ...
                 if l==1  
                    test_query=['{"patient_id":"' patients{i} '","electrode":"' electrodes{k} '","file_block":"' file_block{j} '","type":"2"}'];
                 end;
                 if l==2
                    test_query=['{"patient_id":"' patients{i} '","electrode":"' electrodes{k} '","file_block":"' file_block{j} '","type":"5"}'];
                 end;
                 if l==3
                    test_query=['{"patient_id":"' patients{i} '","electrode":"' electrodes{k} '","file_block":"' file_block{j} '","type":"5","freq_pk": {$gt:350} }'];
                 end;
                 num_records = count(conn,collection,'Query',test_query);

                 if l==1
                 rates(k,1)=(numel(documents)+num_records)/(documents(1).r_duration/60);
                 end;
                 if l==2
                 rates(k,2)=(numel(documents)+num_records)/(documents(1).fr_duration/60);
                 end;
                 if l==3
                 rates(k,3)=(numel(documents)+num_records)/(documents(1).fr_duration/60);
                 end;
