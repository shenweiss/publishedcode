function [masterraster,MDLdata,slowangles,deltaangles,ieegslowang,ieegdeltang,ieegslowZ,ieegslowZbl]=FRONOanalysisBComm1(FRONOchan,hfo_table)
...
for i=1:numel(FRONOchan(:,1)) 
    ...
    if i>1 
        if ~strcmp(FRONOchan.B{i},FRONOchan.B{i-1})
            ...
            HFOstats=1;
            test_query=['{"patient_id":"' FRONOchan.A{i} '","type":"' num2str(4) '","electrode":"' FRONOchan.B{i} '" }']; 
        else
            HFOstats=0;
        end;
     else
...
parfor j=1:numel(units(:,1))
       ...
       slowZ(j)=getfield(units,{j},'slowZ');
end;
dsraster_t=[];
for j=1:numel(HFO(:,1))
       counter=counter+1;
       HFO_start=start_t(j);
       unit_ts=round(unit_ts,3); % this fixes the bug
       ...
       angle_begin2=round(HFO_start,3)-0.5;
       angle_end2=round(HFO_start,3);
       raster=zeros(1,2000);
       dummyHFO=[HFO_begin:0.001:(HFO_end-0.001)];
       dummyAngle=[angle_begin:0.001:angle_end];
       dummyAngle2=[angle_begin2:0.001:angle_end2];
       [C,IA,IB] = intersect(unit_ts,dummyHFO);
       [C2,IA2,IB2] = intersect(unit_ts,dummyAngle);
       [C3,IA3,IB3] = intersect(unit_ts,dummyAngle2);
       slowangles{counter}=slowphase(IA2);
       deltaangles{counter}=deltaphase(IA2);
       ieegslowang{counter}=ieegslowphase(IA2);
       ieegdeltang{counter}=ieegdeltaphase(IA2);
       ieegslowZ{counter}=slowZ(IA2);
       ieegslowZbl{counter}=slowZ(IA3);
       raster(IB)=1;
       k = gausswin(101);
       z1 = conv(raster,k);
...
MDLdata=table(bl,hfo,hfodiff,patient,electrode,unit,loc,soz,sumu,HFOlevel,power,frequency,preSPIKE,start);
