...
function [output_t] = rate_table(patients)
        ...
        for j=1:numel(file_block)
            ...
            rates=[];
               for k=1:numel(electrodes)                  
                  ...
                  for l=1:5
                   if l~=3   
                    ...
                    documents = find(conn,collection,'query',test_query);
                     ...
                     if l==1
                        for m=1:numel(documents)
                            ripple_sf=horzcat(ripple_sf,[documents(m).start_t:0.0001:documents(m).finish_t]);
                        end;
                    end;
                    if l==2
                        for m=1:numel(documents)
                    ...
                    if l==4
                        ripple_sf=round(ripple_sf,4);
                        for m=1:numel(documents)
                            fr_time=[documents(m).start_t:0.0001:documents(m).finish_t];
                            fr_time=round(fr_time,4);
                            [idx]=intersect(ripple_sf,fr_time);
                            if ~isempty(idx)
                                rfr=rfr+1;
                            end;
                        end;
                 ...
                 soz(k)=soztemp;
                 collection = "HFOs";
                 if isempty(documents(1).resected)
                     mt(k)=0;
                 else
