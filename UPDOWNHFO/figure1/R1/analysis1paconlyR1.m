    ...
    electrodes=distinct(conn,collection,"channame",'query',test_query)
         for j=1:numel(electrodes)
             ...
             for k=1:numel(unitnums)
                 ...
                 % calculate circ mean by calculating normalized rose plot
                 
                 [t, r] = rose(slowphase, 72); % 100 is desired number of bins. Set as needed
                 idx=[2:4:288];
                 r=r(idx);
                 t=t(idx);
                 [~,idx]=min(r);
...
output_t=table(patient,electrode,unitnum,soz,loc,maxUP,UP_p,UP_z,DOWN);