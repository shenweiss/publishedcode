function [masterraster,MDLdata,slowangles,deltaangles,FzdeltaZ,FzslowZ]=FRanalysisBraincomm(FRCHAN,hfo_table)
...
for j=1:numel(HFO(:,1))
       ...
       FzdeltaZ{counter}=deltaZ(IA2);
       FzslowZ{counter}=slowZ(IA2);
       raster(IB)=1;
       k = gausswin(101);
       z1 = conv(raster,k);
       ...
       latency=vertcat(latency,spike_latency(j));
...
MDLdata=table(bl,hfo,hfodiff,patient,electrode,unit,loc,soz,HFOlevel,power,frequency,preR,preFR,pcode,latency);
